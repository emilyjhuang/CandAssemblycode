...
void l1_initialize()
{
  ...
  for (int i = 0; i < L1_NUM_CACHE_SETS; i++) {
        for (int j = 0; j < L1_LINES_PER_SET; j++) {
            l1_cache[i].lines[j].v_r_d_tag = 0; // Clearing the entire 64-bit field.
        }
    }
}
		     ...
		     uint8_t control, uint64_t *read_data, uint8_t *status)
{
  ...
  *status = 0; // Default to cache miss
  ...
  for (int i = 0; i < L1_LINES_PER_SET; i++) {
        uint64_t current_tag = l1_cache[set_index].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK;
        uint64_t valid_bit = l1_cache[set_index].lines[i].v_r_d_tag & L1_VBIT_MASK;

        if (valid_bit && (current_tag == tag)) { // Cache hit
            l1_cache[set_index].lines[i].v_r_d_tag |= L1_RBIT_MASK; // Set reference bit
            *status = L1_CACHE_HIT_MASK;

            if (control & 0x1) { // Read operation
                *read_data = l1_cache[set_index].lines[i].cache_line[word_offset];
            }

            if (control & 0x2) { // Write operation
                l1_cache[set_index].lines[i].cache_line[word_offset] = write_data;
                l1_cache[set_index].lines[i].v_r_d_tag |= L1_DIRTYBIT_MASK; // Set dirty bit
            }
		    ...
		    uint8_t *status)
{
  ...
  uint64_t chosen_index = UNINITIALIZED;
  *status = 0; // Default to no write-back required
  ...
  // CODE HERE
  for (int i = 0; i < L1_LINES_PER_SET; i++) {
        uint64_t v_r_d = l1_cache[set_index].lines[i].v_r_d_tag;
        if (!(v_r_d & L1_VBIT_MASK)) { // Valid bit = 0
            chosen_index = i;
            break;
        } 
        // Store the first encountered index for each category
        if (!(v_r_d & L1_RBIT_MASK) && !(v_r_d & L1_DIRTYBIT_MASK)) {
            if (r0_d0_index == UNINITIALIZED) r0_d0_index = i;
        } else if (!(v_r_d & L1_RBIT_MASK) && (v_r_d & L1_DIRTYBIT_MASK)) {
            if (r0_d1_index == UNINITIALIZED) r0_d1_index = i;
        } else if ((v_r_d & L1_RBIT_MASK) && !(v_r_d & L1_DIRTYBIT_MASK)) {
            if (r1_d0_index == UNINITIALIZED) r1_d0_index = i;
        }
    }
    if (chosen_index == UNINITIALIZED) {
        chosen_index = (r0_d0_index != UNINITIALIZED) ? r0_d0_index :
                       (r0_d1_index != UNINITIALIZED) ? r0_d1_index :
                       (r1_d0_index != UNINITIALIZED) ? r1_d0_index :
                       0; // Fallback: overwrite the first line
    }

        // Check if the dirty bit is set for the chosen entry
        uint64_t v_r_d = l1_cache[set_index].lines[chosen_index].v_r_d_tag;
    if (v_r_d & L1_DIRTYBIT_MASK) {
        uint64_t evicted_tag = v_r_d & L1_ENTRY_TAG_MASK;
        *evicted_writeback_address = (evicted_tag << L1_ADDRESS_TAG_SHIFT) | (set_index << L1_SET_INDEX_SHIFT);
        memcpy(evicted_writeback_data, l1_cache[set_index].lines[chosen_index].cache_line, sizeof(uint64_t) * WORDS_PER_CACHE_LINE);
        *status = 1; // Indicate write-back needed
    }

        // Insert the new line
        l1_cache[set_index].lines[chosen_index].v_r_d_tag = (tag & L1_ENTRY_TAG_MASK) | L1_VBIT_MASK; // Set tag and valid bit
        memcpy(l1_cache[set_index].lines[chosen_index].cache_line, write_data, sizeof(uint64_t) * WORDS_PER_CACHE_LINE);
...
void l1_clear_r_bits()
{
  ...
  for (int i = 0; i < L1_NUM_CACHE_SETS; i++) {
        for (int j = 0; j < L1_LINES_PER_SET; j++) {
            l1_cache[i].lines[j].v_r_d_tag &= ~L1_RBIT_MASK; // Clear reference bit
        }
    }
