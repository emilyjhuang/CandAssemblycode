...
void memory_handle_l1_miss(uint64_t address)  
{
  ...
  uint8_t l2_status;
  uint64_t l2_read_data[WORDS_PER_CACHE_LINE];
  l2_cache_access(address, 0, READ_ENABLE_MASK, l2_read_data, &l2_status);
  ...
  l1_insert_line(address, l2_read_data, &evicted_writeback_address, evicted_writeback_data, &l2_status);
  ...
  if (l2_status == 1) {
      l2_cache_access(evicted_writeback_address, evicted_writeback_data, WRITE_ENABLE_MASK, l2_read_data, &l2_status);

      if (l2_status == 0) {
          memory_handle_l2_miss(evicted_writeback_address, WRITE_ENABLE_MASK);
          l2_cache_access(evicted_writeback_address, evicted_writeback_data, WRITE_ENABLE_MASK, NULL, &l2_status);
      }
  }
}
