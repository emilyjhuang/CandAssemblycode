...
void l2_initialize()
{
  ...
  for (int i = 0; i < L2_NUM_CACHE_ENTRIES; i++) {
        l2_cache[i].v_d_tag = 0; // Clears the entire v_d_tag field
    }
}
		     ...
		     uint8_t control, uint64_t read_data[], uint8_t *status)
{
  ...
  // CODE HERE
  uint32_t index = (address & L2_INDEX_MASK) >> L2_INDEX_SHIFT;
  uint64_t tag = (address & L2_ADDRESS_TAG_MASK) >> L2_ADDRESS_TAG_SHIFT;

    uint32_t v_d_tag = l2_cache[index].v_d_tag;
    uint64_t entry_tag = v_d_tag & L2_ENTRY_TAG_MASK;
    *status = 0;

    if ((v_d_tag & L2_VBIT_MASK) && (entry_tag == tag)) { // Cache hit
        *status = L2_HIT_STATUS_MASK;
        if (control & 0x01) { // Read enable
            for (int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
                read_data[i] = l2_cache[index].cache_line[i];
            }
        }
        if (control & 0x02) { // Write enable
            for (int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
		    ...
		    uint8_t *status)
{
  ...
  // CODE HERE
  L2_CACHE_ENTRY *entry = &l2_cache[index];
    *status = 0;    
  ...
  // CODE HERE
  if (entry->v_d_tag & L2_VBIT_MASK) {
        if (entry->v_d_tag & L2_DIRTYBIT_MASK) { // Write-back needed
            *evicted_writeback_address = ((entry->v_d_tag & L2_ENTRY_TAG_MASK) << L2_ADDRESS_TAG_SHIFT) | (index << L2_INDEX_SHIFT);
            for (int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
                evicted_writeback_data[i] = entry->cache_line[i];
            }
            *status = 1;
        }
    }
  ...
  // CODE HERE
  for (int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
        entry->cache_line[i] = write_data[i];
    }
    entry->v_d_tag = (tag & L2_ENTRY_TAG_MASK) | L2_VBIT_MASK; // Set tag and valid bit, clear dirty bit
}
